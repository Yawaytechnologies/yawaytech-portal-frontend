// src/components/employee/FinanceEmployees.jsx
import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import { fetchFinanceEmployees } from "../../redux/actions/financeActions";

export default function FinanceEmployees() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { employees = [], loading, error } = useSelector((s) => s.finance || {});

  useEffect(() => {
    dispatch(fetchFinanceEmployees());
  }, [dispatch]);

  return (
    <div className="p-6 bg-[#f4f6fa] min-h-screen">
      <h1 className="text-3xl font-bold mb-8 text-[#223366]">Finance Employees</h1>

      {loading && <p>Loading Finance employees...</p>}
      {error && <p className="text-red-500">{String(error)}</p>}

      {!loading && !error && employees.length === 0 && (
        <p className="text-gray-600">No Finance employees found.</p>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {employees.map((emp, idx) => {
          ...
          return (
            <div
              key={key}
              onClick={() => id && navigate(`/employees/finance/${id}`)}
