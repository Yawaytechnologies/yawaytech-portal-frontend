...
const getAvatarSrc = (row) => {
  const avatarRaw = row.profile || row.profile_picture || row.avatar || null;

  if (!avatarRaw) return null;

  if (typeof avatarRaw === "string" && avatarRaw.startsWith("data:")) {
    return avatarRaw;
  }

  return `data:image/jpeg;base64,${String(avatarRaw).trim()}`;
};
