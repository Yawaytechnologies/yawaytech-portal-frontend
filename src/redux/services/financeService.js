// src/redux/services/financeService.js
const API_BASE = (
...
/** Normalize raw Finance employee object into UI-friendly shape */
export const normalizeFinanceEmployee = (e = {}) => {
...
/* -------- fetch list -------- */
export async function fetchFinanceEmployeesAPI({ limit = 100, offset = 0 } = {}) {
  const url = `${API_BASE}api/dashboard/employees?department=Finance&limit=${limit}&offset=${offset}`;
  const res = await fetch(url, { headers: { Accept: "application/json" } });
  const text = await res.text();

  if (!res.ok) throw new Error(`Finance API ${res.status}: ${text || res.statusText}`);
  ...
  return list.map(normalizeFinanceEmployee);
}
...
const singleCandidates = (enc) => [
  ...
  `${API_BASE}api/finance/${enc}`,
  `${API_BASE}api/employees/by-code/${enc}`,
  `${API_BASE}api/employees/by-employee-id/${enc}`,
];

export async function fetchFinanceEmployeeByIdAPI(employeeId) {
  ...
  for (const url of singleCandidates(enc)) {
    try {
      ...
      return normalizeFinanceEmployee(payload, id);
    } catch {
      continue;
    }
  }
  ...
  // 2) list fallback
  const list = await fetchFinanceEmployeesAPI({ limit: 1000, offset: 0 });
  ...
  return normalizeFinanceEmployee({ employee_id: id, name: id }, id);
}
