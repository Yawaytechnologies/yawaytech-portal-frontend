// src/components/EmployeeOverview/FinanceOverview.jsx
import React, { useEffect, useMemo } from "react";
import { useParams, useNavigate, Link } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { fetchFinanceById } from "../../redux/actions/financeOverviewAction";
import {
  MdEmail,
  MdPhone,
  MdBadge,
...
export default function FinanceOverview() {
  ...
  useEffect(() => {
    ...
    if (!id) {
      navigate("/employees/finance");
      return;
    }
    dispatch({ type: "FINANCE_DETAIL_RESET" });
    dispatch(fetchFinanceById(id));
  ...
  const M = useMemo(() => {
    ...
    return {
      ...
      name: val(e.name),
      avatar,
      title: val(e.designation || e.jobTitle || e.role || "Finance"),
      email: val(e.email),
        ...
        <div className="bg-white rounded-2xl shadow-xl border-t-4 border-[#FF5800] p-4 sm:p-6 md:p-8">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              <Link
                to={`/employees/finance/${encodeURIComponent(M.id)}/worklog`}
