...
export const loginAdminService = async ({ adminId, password }) => {
  ...
  const loginRes = await fetch(
    ...
    json("POST", { admin_id: id, password }),
  );
...
export const loginEmployeeService = async ({ employeeId, password }) => {
  ...
  const loginRes = await fetch(
    `${API_BASE}/api/api/employee/login`,
    json("POST", { employee_id: id, password }),
  );
  if (!loginRes.ok) throw new Error(await parseError(loginRes));
  const data = await loginRes.json();
  const token = data?.access_token;
  ...
  fetchWithTimeout(`${API_BASE}/api/api/employee/me`, json("GET", null, token))
    .then(async (r) => (r.ok ? r.json() : null))
    .then((profile) => {
      if (profile)
