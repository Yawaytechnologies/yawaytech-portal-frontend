// src/redux/services/marketingService.js
const API_BASE = (
...
/** Normalize raw Marketing employee object into UI-friendly shape */
export const normalizeMarketingEmployee = (e = {}) => {
  ...
  return {
    ...
    department: e.department ?? "Marketing",
...
/* -------- fetch list -------- */
export async function fetchMarketingEmployeesAPI({ limit = 100, offset = 0 } = {}) {
  const url = `${API_BASE}api/dashboard/employees?department=Marketing&limit=${limit}&offset=${offset}`;
  const res = await fetch(url, { headers: { Accept: "application/json" } });
  const text = await res.text();

  if (!res.ok) throw new Error(`Marketing API ${res.status}: ${text || res.statusText}`);
  ...
  return list.map(normalizeMarketingEmployee);
}
...
const singleCandidates = (enc) => [
  ...
  `${API_BASE}api/marketing/${enc}`,
  `${API_BASE}api/employees/by-code/${enc}`,
  `${API_BASE}api/employees/by-employee-id/${enc}`,
];

export async function fetchMarketingEmployeeByIdAPI(employeeId) {
  ...
  for (const url of singleCandidates(enc)) {
    try {
      ...
      return normalizeMarketingEmployee(payload, id);
    } catch {
      continue;
    }
  }
  ...
  // 2) list fallback
  const list = await fetchMarketingEmployeesAPI({ limit: 1000, offset: 0 });
  ...
  return normalizeMarketingEmployee({ employee_id: id, name: id }, id);
}
