...
function normalizeProfile(e) {
  if (!e) return null;
  // priority: profile_picture (base64), then profile/photo_url/avatar
...
/**
 * Normalize raw HR employee object into UI-friendly shape
 */
export const normalizeHREmployee = (e = {}) => {
...
/* ---------------- fetch list ---------------- */
export async function fetchHREmployeesAPI({ limit = 100, offset = 0 } = {}) {
  const url = `${API_BASE}api/dashboard/employees?department=HR&limit=${limit}&offset=${offset}`;
  const res = await fetch(url, { headers: { Accept: "application/json" } });
  const text = await res.text();

  if (!res.ok) throw new Error(`HR API ${res.status}: ${text || res.statusText}`);
  ...
  return list.map(normalizeHREmployee);
}
...
export async function fetchHREmployeeByIdAPI(employeeId) {
  const id = String(employeeId || "").trim();
  if (!id) throw new Error("Invalid employee id");

  const enc = encodeURIComponent(id);

  // 1) Try single-item endpoints
  for (const url of singleCandidates(enc)) {
    try {
      const res = await fetch(url, { headers: { Accept: "application/json" } });
      if (!res.ok) continue;
      const payload = await res.json();
      return normalizeHREmployee(payload, id);
    } catch {
      continue;
    }
  }

  // 2) Fallback: fetch list and search
  try {
    const list = await fetchHREmployeesAPI({ limit: 1000, offset: 0 });
    ...
    // Not found — return minimal normalized object
    return normalizeHREmployee({ employee_id: id, name: id }, id);
  } catch (err) {
    throw new Error(`Could not fetch HR employee: ${err?.message ?? err}`);
  }
}
